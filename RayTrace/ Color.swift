...
func rgbColor(from nsColor: NSColor) -> RGBAColor {
	
	guard let coreColour = CIColor(color: nsColor) else { return .white}

	let a = UInt8(coreColour.alpha * 255)
	let r = UInt8(coreColour.red * 255)
	let g = UInt8(coreColour.green * 255)
	let b = UInt8(coreColour.blue * 255)
	
	return .init(a: a, r: r, g: g, b: b)
}
...
extension NSColor {
	...
	func withLuminosity(_ newLuminosity: CGFloat) -> NSColor {
		// 1 - Convert the RGB values to the range 0-1
		guard let coreColour = CIColor(color: self) else { return self}
		...
		if saturation == 0 {
			return NSColor(red: 1.0 * luminosity, green: 1.0 * luminosity, blue: 1.0 * luminosity, alpha: alpha)
		}
